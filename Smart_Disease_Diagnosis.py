...
import pandas as pd
import database
from database import get_prediction_history  # ✅ Directly import function
...
with st.sidebar:
    ...
    selected = option_menu(
        "Disease Prediction",
        ["Diabetes Prediction", "Heart Disease Prediction", "Parkinsons Prediction", "Lung Cancer Prediction", "Hypo-Thyroid Prediction", "Prediction History"],
...
def load_models():
    return {
        'diabetes': joblib.load(open('Models/diabetes_model.sav', 'rb')),
        'heart_disease': joblib.load(open('Models/heart_disease_model.sav', 'rb')),
        'parkinsons': joblib.load(open('Models/parkinsons_model.sav', 'rb')),
        'lung_cancer': joblib.load(open('Models/lungs_disease_model.sav', 'rb')),
        'thyroid': joblib.load(open('Models/Thyroid_model.sav', 'rb'))
...
# ✅ Display Prediction History as a Table
if selected == "Prediction History":
    st.markdown("<h1 style='text-align:center;'></h1>", unsafe_allow_html=True)

    user = st.session_state.get("user_name", "Guest")  # ✅ Fetch logged-in user
    history = database.get_prediction_history(user)  # ✅ Fetch history from database

    if not history:
        st.warning("⚠️ No past predictions found. Try making a new prediction first.")
    else:
        # ✅ Convert history into a DataFrame
        df = pd.DataFrame(history, columns=["Disease", "Result", "Probability (%)", "Date & Time"])
